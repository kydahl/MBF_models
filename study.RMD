...
if (PRCC_calc) {
  ...
  vary_parameters <- select(parameters, -eta) %>%
    cross_join(tibble(eta = 1/(1440 * seq(1, 28, length.out = 28))))
  ...
  vary_parameters <- select(parameters, -mu) %>%
    cross_join(tibble(mu = 1/(1440 * seq(1, 60, length.out = 2 * 60))))
  ...
  vary_parameters <- select(parameters, -lambdaQ) %>%
    cross_join(tibble(lambdaQ = 1/(1440 * seq(1/60, 60, length.out = 2 * 60))))
...
```{r R0-mono-check, message=FALSE, warning=FALSE}
...
```{r PRCC_var_test, message=FALSE, warning=FALSE}
