...
```{r empirical-fitting, message=FALSE, warning=FALSE}
...
if (redo_fitting) {
  ...
  plan(multisession, workers = 10, gc = FALSE)
  # # Set up progress bar
  # handlers(global = TRUE)
  # handlers("cli")
  # Create temporary function to create table
  temp_func = function(indices) {
  ...
  mech_params_df = temp_func(1:dim(mech_res$samples)[1]) %>% 
    filter(lambda_L < 60)
...
for (i in 1:dim(mech_params_df)[1]) {
...
```{r base-R0-PRCC, message=FALSE, warning=FALSE}
# Posterior R0 PRCC distribution calculations
sens_res = 10000
sens_range = 0.75
...
for (k in 1:dim(sens_params)[1]) {
  ...
  Rs = R0_calc(PH_mat, "Mechanistic", parameters, provide_params = sens_params[k,])
...
R0_PRCC_func <- function(vary_parameters) {
  # my_multisession = tweak(multisession, workers = (floor(availableCores()/2)-2))
  # plan(list(my_multisession,my_multisession))

  plan(multisession, workers = availableCores())
  indices = 1:dim(vary_parameters)[1]
  
  # Set up progress bar
  handlers(global = TRUE)
  handlers("cli")
  p = progressor(along = 1:dim(sens_params)[1])
  ...
  foreach(
    ...
    .combine = "rbind") %do% {
      ...
      R0_sensitivity_df = foreach(
        k = 1:dim(sens_params)[1],
        .combine = "rbind") %dofuture% {
          # Iterate progress bar
          p(sprintf("i=%g", k))
          PH_mat = mech_params_to_mat(sens_params[k,])
          Rs = R0_calc(PH_mat, "Mechanistic", vary_parameters[i,], provide_params = sens_params[k,])
...
library(doFuture)
PRCC_var_test = foreach(i = 1:10,
                        .combine = "rbind") %do% {
                          print(i)
                          test = R0_PRCC_func(parameters)
                          plan(sequential)
                          test
                        }

write_csv(PRCC_var_test, "./data/PRCC_variance_test.csv")
...
vary_parameters <- select(parameters, -eta) %>%
  cross_join(tibble(eta = 1/(1440 * seq(1, 7, length.out = 42))))

gc()
library(doFuture) # Faster parallel processing
R0_PRCC_df = R0_PRCC_func(vary_parameters) %>% 
...
R0_PRCC_df %>%
  ...
  ggplot() +
  geom_point(aes(x = EIP, y = PRCC_R0), alpha = 0.3) +
  geom_line(aes(x = EIP, y = PRCC_R0)) +
  geom_hline(aes(yintercept = 0), color = "red") +
