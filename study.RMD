...
knitr::opts_chunk$set(
  echo = FALSE, cache = FALSE, warning = FALSE, fig.pos = "H", dev = c("png", "pdf")
)
# Load Libraries----------------------------------------------------------------
library(matrixdist)
library(tidyverse)
...
param_table$Label <- factor(param_table$Label, levels = c("Questing rate", "Seek-new-host probability", "Landing success probability", "Landing rate", "Probing success probability", "Probing rate", "Ingestion success probability", "Ingesting rate"))

kable(select(param_table, -c(name, Label)) %>% rename(Value = value))
```
...
eta = 1/(10 * 1440) # 10 days for infection to develop in vector
...
KL = 3000
rhoL = 1/(12 * 1440) # 12 day larval development period (from egg to adult)
muL = 1/(20 * 1440) # 62.5% probability of larval survival
...
parameters = tibble(
  eta = eta, mu = mu, gammaV = gammaV, gammaR = gammaR, gammaH = gammaH, muH = muH, KH = KH, KL = KL, rhoL = rhoL, muL = muL, varPhi = varPhi, epi_parms
)

nice_parameter_table = tibble(
  pivot_longer(parameters, everything()),
  Symbol = c("$\\eta$", "$\\mu$", "$\\gamma_V$", "$\\gamma_R$", "$\\gamma_H$", "$\\mu_H$", "$K_H$", "$K_L$", "$\\rho_L$", "$\\mu_L$", "$\\varphi$", "$\\beta_P$", "$\\beta_G$", "$\\beta_H$", "$\\beta_V$"),
  Description = c("Extrinsic incubation rate", 
                  "Mosquito mortality rate",
                  "Oviposition exit rate",
...
kable(select(nice_parameter_table, -name) %>%  relocate(Symbol, Description, Value = value), digits = c(0,0,6))
```
...
set_alpha = c(1, 0, 0, 0) # initial probability vector
...
test_data = direct_samps[1:min(sample_size, length(direct_samps))]
```
...
\[
A = \begin{bmatrix}-\lambda_{Q} & \lambda_{Q} & 0 & 0\\
f\left(1-p_{L}\right)\lambda_{L} & -\lambda_{L}+\left(1-f\right)\left(1-p_{L}\right)\lambda_{L} & p_{L}\lambda_{L} & 0\\
f\left(1-p_{P}\right)\lambda_{P} & \left(1-f\right)\left(1-p_{P}\right)\lambda_{P} & -\lambda_{P} & p_{p}\lambda_{P}\\
f\left(1-p_{G}\right)\lambda_{G} & \left(1-f\right)\left(1-p_{G}\right)\lambda_{G} & 0 & -\lambda_{G}
\end{bmatrix}
\]
...
```{r plot-simulation}
ggplot(as.data.frame(x = direct_samps), aes(x=direct_samps / 1440)) +
  geom_density(fill = "blue", alpha = 0.5) + 
  scale_x_continuous("Time before ending blood-feeding (days)", expand = c(0,0)) +
...
parameters = tibble(
  eta = eta, mu = mu, gammaR = gammaR, gammaV = gammaV, gammaH = gammaH, muH = muH, KH = KH, KL = KL, rhoL = rhoL, muL = muL, varPhi = varPhi, epi_parms
)
...
for (j in 1:num_params) {
  ...
  vary_range = seq(max(eps, (1 - sens_range)), max_val, length.out = num_vary)
...
R0_sensitivity_df <- tibble(R0 = as.double(), R = as.double(),
                            lambda_Q = as.double(), p_L = as.double(), lambda_L = as.double(), p_P = as.double(),
...
with_progress({
  p = progressor(along = 1:num_vary)
  
  R0_sensitivity_df = foreach(j = 1:num_params,
                              .combine = 'rbind') %do% {
                                foreach(i = 1:num_vary,
...
R0_mono_plot <- test %>%
  filter(R0 > 0) %>% 
  right_join(select(param_table, name, Label), by = "name") %>% 
  ggplot(aes(x = value, y = R0)) +
  geom_line(lwd = 1) +
  # geom_point(data = test2, aes(color = "mech"), alpha = 0.3) +
  geom_point(data = right_join(base_points, select(param_table, name, Label), by = "name"), aes(color = "actual"), size = 3) +
...
Rs_mono_plot <- test %>%
  ...
  scale_color_manual("Type",
                     ...
                     values = c("actual"="green", "mech"="blue"),
                     labels = c("actual", "mech")) +
  scale_y_continuous(name = TeX("Basic offspring number, $R$")) +
  scale_x_continuous(name = c()) +
  facet_wrap( ~ Label, scales = "free", ncol = 2) +
  theme_minimal_grid()
Rs_mono_plot
...
```{r monotonicity_both_Rs-plot}

# Scaling coefficient for the two y-axes
coeff = mean(mutate(filter(test, R0 >0), ratio = R / R0)$ratio, na.rm = T)
coeff = 50

both_Rs_mono_plot <- test %>%
  filter(R0 > 0) %>% 
  mutate(ratio = R / R0) %>% 
  right_join(select(param_table, name, Label), by = "name") %>% 
  ggplot(aes(x = value)) +
  geom_line(aes(y = R0), color = "red", lwd = 1) +
  geom_line(aes(y = R / coeff), color = "blue", lwd = 1) + 
  # geom_point(data = test2, aes(color = "mech"), alpha = 0.3) +
  # geom_point(data = right_join(base_points, select(param_table, name, Label), by = "name"), aes(y = R0, color = "actual"), size = 3) +
  # geom_point(data = right_join(base_points, select(param_table, name, Label), by = "name"), aes(y = R / coeff, color = "actual"), size = 3) +
  scale_color_manual("Type",
                     breaks = c("actual"="green", "mech"="blue"),
                     values = c("actual"="green", "mech"="blue"),
                     labels = c("actual", "mech")) +
  scale_y_continuous(name = TeX("Basic reproduction number, $R_0$"),
                     sec.axis = sec_axis(trans = ~.*coeff, 
                                         name = TeX("Basic offspring number, $R$"))
  ) +
  scale_x_continuous(name = c()) +
  facet_wrap( ~ Label, scales = "free", ncol = 2) +
  theme_minimal_grid() +
...
ggsave("figures/Rs_comparison.png", plot = both_Rs_mono_plot, 
       width = 13.333, height = 6.5, units = "in")

```

```{r monotonicity_isolate_pG-plot}

coeff = 1

just_pG_mono_plot <- test %>%
  filter(name == "p_G") %>% 
  filter(R0 > 0) %>% 
  mutate(ratio = R / R0) %>% 
  right_join(select(param_table, name, Label), by = "name") %>% 
  ...
  geom_line(aes(y = R / coeff), color = "blue", lwd = 1) + 
  # geom_point(data = test2, aes(color = "mech"), alpha = 0.3) +
  # geom_point(data = right_join(base_points, filter(select(param_table, name, Label), name == "p_G"), by = "name"), aes(y = R0, color = "actual"), size = 3) +
  # geom_point(data = right_join(base_points, filter(select(param_table, name, Label), name == "p_G"), by = "name"), aes(y = R, color = "actual"), size = 3) +
  scale_color_manual("Type",
                     breaks = c("actual"="green", "mech"="blue"),
                     values = c("actual"="green", "mech"="blue"),
                     labels = c("actual", "mech")) +
  ...
  # facet_wrap( ~ Label, scales = "free", ncol = 2) +
  theme_minimal_grid() +
  ggtitle("Ingestion success probability") +
  theme(
    axis.title.y = element_text(color = "red", size=13),
    axis.title.y.right = element_text(color = "blue", size=13), 
...
ggsave("figures/Rs_comparison_pGfocus.png", plot = just_pG_mono_plot, 
       width = 13.333, height = 6.5, units = "in")
...
Table of values
```{r baseline-prcc-plot, message=FALSE, warning=FALSE, fig.cap = 'Table 3: Baseline PRCC values'}


# Labels
param_labels = select(param_table, name, Label, Prefix, Type) %>% 
  arrange(Label) %>% 
  add_row(name = "dummy", Label = "Dummy variable", Prefix = "Dummy", Type = "Rate")


temp = read_rds("./data/PRCC_variance_test.rds") %>% 
  filter(iterate == 1) %>%
  right_join(param_labels, by = "name")

temp$Label <- factor(temp$Label, levels = c("Dummy variable", "Questing rate", "Seek-new-host probability", "Landing success probability", "Landing rate", "Probing success probability", "Probing rate", "Ingestion success probability", "Ingesting rate"))

dummy_replace = temp %>% 
  mutate(PRCC_R0 = filter(temp, name == "dummy")$PRCC_R0) %>% 
  mutate(x = Inf)

dummy_replace = add_row(dummy_replace, mutate(dummy_replace, x = -Inf))

# Labels

temp$Prefix = factor(temp$Prefix, levels = c("Dummy", "Questing", "Seeking", "Landing", "Probing", "Ingesting"))
temp$Type = factor(temp$Type, levels = c("Rate", "Probability"))


# Plot baseline PRCC values
baseline_PRCC_plot <- temp %>% 
  ggplot() +
  geom_col(aes(x = Label, y = PRCC_R0, fill = Prefix, linetype = Type), 
           color = "black", lwd = 1) +
  geom_hline(aes(yintercept = 0), lwd = 1) +
  # geom_hline(data = filter(temp, name == "dummy"), aes(yintercept = PRCC_R0)) +
  # geom_ribbon(data = dummy_replace, aes(ymin = -PRCC_R0, ymax = PRCC_R0, x, y = NULL), alpha = 0.5) +
  # ggtitle("Baseline PRCC results") +
  scale_x_discrete("") + 
  scale_y_continuous(TeX("$R_0$ PRCC values")) + #, limits = c(-1,1)) +
  scale_fill_manual("Parameter", values = c("grey", c4a("poly.dark24", 5))) +
  theme_minimal_hgrid(16) +
  theme(axis.text.x = element_text(angle = 30, vjust = 1, hjust = 1))

baseline_PRCC_plot

ggsave("figures/baseline_PRCC.png", plot = baseline_PRCC_plot, 
       width = 13.333, height = 6.5, units = "in")
...
R0_PRCC_func <- function(vary_parameters, progress_bool = FALSE) {
  ...
  for (j in 1:num_params) {
    ...
    if (colnames(param_set[j]) %in% c("f", "p_L", "p_P", "p_G")) {
      ...
      max_val = (1 + sens_range) * param_set[j][[1]]
    }
    
    transformed_LHS_mat[,j] = qunif(LHS_mat[,j],
                                    min = max(0, (1 - sens_range)) * param_set[j][[1]],
                                    max = max_val)
  }
  ...
  if (progress_bool == TRUE) {
    ...
    progress_chunk = if_else(progress_length < 10, 1, 10)
  ...
  plan(sequential)
  # print(1/vary_parameters$eta/1440)
  return(out)
}
...
if (PRCC_calc) {
  ...
  repeat_num = 10
  for (i in 1:(repeat_num - 1)) {rep_params = rbind(rep_params, parameters)}

  PRCC_var_test = R0_PRCC_func(rep_params, progress_bool = TRUE) %>%
    mutate(iterate = floor((row_number() - 1) / 9) + 1)
  ...
  vary_parameters <- select(parameters, -eta) %>%
    cross_join(tibble(eta = 1/(1440 * seq(1, 21, length.out = 100))))
  ...
  write_rds(R0_PRCC_df_EIP, "./data/vary_EIP_PRCC_vals.rds")
    ...
    # Calculate PRCCs as resting time / oviposition site availability is varied
  print("Calculating PRCCs as gamma_V is varied")
  parameters = tibble(
    eta = eta, mu = mu, gammaR = gammaR, gammaV = gammaV, gammaH = gammaH, muH = muH, KH = KH, KL = KL, rhoL = rhoL, muL = muL, varPhi = varPhi, epi_parms
  )
  vary_parameters <- select(parameters, -gammaV) %>%
    cross_join(tibble(gammaV = 1/(1440 * seq(1, 21, length.out = 21))))
  
  R0_PRCC_df_gamma = R0_PRCC_func(vary_parameters, progress_bool = TRUE) %>% 
    select(name, PRCC_R0, PRCC_R, gammaV)
  gc(verbose = FALSE)
  
  write_rds(R0_PRCC_df_gamma, "./data/vary_gamma_PRCC_vals.rds")
  
  # Calculate PRCCs as lifespan is varied
  print("Calculating PRCCs as lifespan is varied")
  parameters = tibble(eta = eta, mu = mu, gammaR = gammaR, gammaV = gammaV, gammaH = gammaH, 
                      muH = muH, KH = KH, KL = KL, rhoL = rhoL, muL = muL, 
                      varPhi = varPhi, epi_parms)
  
  vary_parameters <- select(parameters, -mu) %>%
    cross_join(tibble(mu = 1/(1440 * seq(1, 30, length.out = 6 * 60))))
  ...
  #   cross_join(tibble(lambdaQ = 1/(1440 * seq(1/60, 10, length.out = 4 * 60))))
  # 
...
# Table of baseline PRCC values
read_rds("./data/PRCC_variance_test.rds") %>% 
  filter(iterate %in% 1) %>%  
  select(name, PRCC_R0, PRCC_R) %>% 
  kable()
...
# Labels
labels = select(param_table, name, Label) %>% 
  arrange(Label) %>% 
  add_row(name = "dummy", Label = "Dummy variable") %>% 
  deframe()
...
read_rds("./data/mono_R0_vals.rds") %>%
  ...
  scale_y_continuous(name = TeX("$\\log(R_0)$")) +
...
plot_df <- read_rds("./data/vary_EIP_PRCC_vals.rds") %>%
  mutate(EIP = 1 / (1440 * eta)) %>%
  right_join(param_labels, by = "name")

plot_df$Prefix = factor(plot_df$Prefix, levels = c("Dummy", "Questing", "Seeking", "Landing", "Probing", "Ingesting"))
plot_df$Type = factor(plot_df$Type, levels = c("Rate", "Probability"))


R0_PRCC_vary_EIP_plot <- filter(plot_df, name != "dummy") %>%
  ggplot(aes(x = EIP)) +
  # geom_point(alpha = 0.3) +
  geom_line(aes(y = PRCC_R0, color = Prefix, linetype = Type), lwd = 1) +
  geom_hline(aes(yintercept = 0), color = "black") +
  geom_ribbon(data = filter(plot_df, name == "dummy"), 
              aes(ymin = -abs(PRCC_R0), ymax = abs(PRCC_R0)),
              fill = c4a("tableau.classic_color_blind", 6)[3],
              alpha = 0.5) +
  scale_color_manual("Parameter", values = c4a("poly.dark24", 5)) +
  scale_x_continuous("Extrinsic incubation period (days)", expand = c(0,0)) +
  scale_y_continuous(TeX("$R_0$ PRCC value"), expand = c(0,0)) +
  theme(axis.ticks.x = element_text(size = 6)) +
  theme_minimal_grid()

R0_PRCC_vary_EIP_plot

ggsave("figures/R0_PRCC_vary_EIP.svg", plot = R0_PRCC_vary_EIP_plot, 
       width = 13.333, height = 6.5, units = "in")
...
plot_df <- read_rds("./data/vary_lf_PRCC_vals.rds") %>%
  mutate(lf = 1/(1440 * mu)) %>% 
  right_join(param_labels, by = "name")
...
R0_PRCC_vary_lf_plot <- filter(plot_df, name != "dummy") %>%
  ggplot(aes(x = lf)) +
  # geom_point(alpha = 0.3) +
  geom_line(aes(y = PRCC_R0, color = Prefix, linetype = Type), lwd = 1) +
  geom_hline(aes(yintercept = 0), color = "black") +
  ...
  scale_color_manual("Parameter",
                     values = c4a("poly.dark24", 5)
                     # breaks = rev(c("lambda_Q", "p_L", "lambda_L", "p_P", "lambda_P", "p_G", "lambda_G", "f", "dummy"))
  ) +
  scale_x_continuous("Mosquito lifespan (days)", expand = c(0,0), limits = c(1, 30)) +
  scale_y_continuous("R0 PRCC value", expand = c(0,0)) +
  # ggtitle("R0 PRCCs as mosquito lifespan is varied") +
  theme(axis.ticks.x = element_text(size = 6)) +
  theme_minimal_grid()

R0_PRCC_vary_lf_plot


ggsave("figures/R0_PRCC_vary_lf.svg", plot = R0_PRCC_vary_lf_plot, 
       width = 13.333, height = 6.5, units = "in")

```

```{r R0-PRCC-vary-gamma, message=FALSE, warning=FALSE}
...
plot_df <- read_rds("./data/vary_gamma_PRCC_vals.rds") %>%
  mutate(rest = 1/(1440 * gammaV)) %>% 
  right_join(param_labels, by = "name")

plot_df$Prefix = factor(plot_df$Prefix, levels = c("Dummy", "Questing", "Seeking", "Landing", "Probing", "Ingesting"))
plot_df$Type = factor(plot_df$Type, levels = c("Rate", "Probability"))
  
R0_PRCC_vary_gamma_plot <- filter(plot_df, name != "dummy") %>%
  ggplot(aes(x = rest)) +
  # geom_point(alpha = 0.3) +
  geom_line(aes(y = PRCC_R0, color = Prefix, linetype = Type), lwd = 1) +
  geom_hline(aes(yintercept = 0), color = "black") +
  ...
  scale_x_continuous("Resting period (days)", expand = c(0,0)) +
  scale_y_continuous("R0 PRCC value", expand = c(0,0)) +
  # ggtitle("R0 PRCCs as mosquito lifespan is varied") +
  theme(axis.ticks.x = element_text(size = 6)) +
...
R0_PRCC_vary_gamma_plot


ggsave("figures/R0_PRCC_vary_gamma.svg", plot = R0_PRCC_vary_gamma_plot, 
       width = 13.333, height = 6.5, units = "in")
```
...
# TODO:
# [] fix the uninformed priors so that the sampler doesn't lock up
...
if (redo_fitting) {
  ...
  for (PH_order in orders) {
    ...
    priors <- uninf_priors_func(mean(1/test_data) * num_transient_states, var(1/test_data), num_transient_states)
    # Gamma prior: reciprocal scale hyperparameters (one per matrix row)
    nu <- priors$nu # / 1000000 #rep(1/mean(test_data), num_states)
    zeta <- priors$zeta[1:num_transient_states]# / 1000000
...
if (redo_fitting) {
  ...
  zeta <- c(
    "Q1" = zeta_ref,
    "L1" = zeta_ref, "L2" = zeta_ref,
    "P1" = zeta_ref, "P2" = zeta_ref, "P3" = zeta_ref,
    "G1" = zeta_ref, "G2" = zeta_ref, "G3" = zeta_ref
  )
  # Gamma priors for shape hyperparameters of model parameters
  nu <- c(
    "Q1" = (lQ) * zeta_ref,
    "L1" = (f * (1-pL) * lL) * zeta_ref, "L2" = (pL*lL) * zeta_ref,
    "P1" = (f * (1 - pP) * lP) * zeta_ref, "P2" = ((1-f) * (1-pP) * lP) * zeta_ref, "P3" = (pP * lP) * zeta_ref,
    "G1" = (f * (1 - pG) * lG) * zeta_ref, "G2" = ((1-f) * (1-pG) * lG) * zeta_ref, "G3" = (pG * lG) * zeta_ref
  )
  ...
  # zeta <- c(
  #   "Q1" = (lQ)^(-1)/nu_ref,
  #   "L1" = (f * (1-pL) * lL)^(-1)/nu_ref, "L2" = (pL*lL)^(-1)/nu_ref,
  #   "P1" = (f * (1 - pP) * lP)^(-1)/nu_ref, "P2" = ((1-f) * (1-pP) * lP)^(-1)/nu_ref, "P3" = (pP * lP)^(-1)/nu_ref,
  #   "G1" = (f * (1 - pG) * lG)^(-1)/nu_ref, "G2" = ((1-f) * (1-pG) * lG)^(-1)/nu_ref, "G3" = (pG * lG)^(-1)/nu_ref
  # )
  
  # alpha is always set to (1,0,0)
  alpha = rep(0, 4); alpha[1] = 1
  ...
  mech_param_func = function(indices) {
    ...
    mech_params_df = foreach(
      ...
      .combine = "rbind") %dofuture% {
        ...
        # !!! [] TODO: rewrite reshape_phtMCMC to include questing
  ...
  mech_params_df = temp %>% 
    filter(lambda_L < 60, lambda_L > 0) %>% 
    arrange(logLik)
  mech_params_df = mech_params_df[1:min(1000, dim(mech_params_df)[1]),]
...
best_fit = filter(mech_params_df, logLik == max(logLik))
out_mat = mech_params_to_mat(select(best_fit, -c(iterate, logLik)))
mech_times = rphtype(compare_res, c(1,0,0,0), out_mat)
...
actual_mech_parms <- tibble(
  lambda_Q = lQ, p_L = pL, lambda_L = lL, p_P = pP, lambda_P = lP, p_G = pG, lambda_G = lG,
  f = f) %>% 
