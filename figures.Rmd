...
test_data = select(plot_df, class_label, R0) %>% 
  filter(class_label != "Mechanistic")
t.test(R0~class_label, var.equal = T, data = test_data)

test_data = select(plot_df, class_label, R0) %>% 
  filter(class_label != "Phenomenological")
t.test(R0~class_label, var.equal = T, data = test_data)


test_data = select(plot_df, class_label, R0) %>% 
  filter(class_label != "Empirical")
t.test(R0~class_label, var.equal = T, data = test_data)
