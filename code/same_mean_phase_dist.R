...
# Questing
pQ = 1
lQ = 1 / (3 * 1440) # 1 / (8 * 60) # 8 hours = 480 minutes
...
R_calc_function_exp <- function(params) {
  with(as.list(params), {
    order = 1
    # A = -1/BFD
    # A = -1/(1/lQ + 1/lL + 1/lP + 1/lG)
    # A = -1/(theta - 1/gammaV - 1/gammaR)
    # theta = 1/lQ + 1/lL + 1/lP + 1/lG
    A = -1 / theta
    b = -A
    ...
    B_star = rhoL * KL * (1 - ((rhoL / (muL + rhoL)) * varPhi / mu)^(-1)) / mu
    
    # B_star = (N_offspring - 1) * KL * ((rhoL / N_offspring) + (muL + rhoL) / varPhi) * (1/(mu+b))
    
    R0 = b * sqrt(betaV * (B_star / KH) * (1 / (muH + gammaH)) * betaH * (eta / (mu + eta)) * (1 / mu))
...
theta_vec = 1440 * seq((1/gammaV + 1/gammaR)/1440, 31, length.out = 1001)
# theta_vec = seq(min_exp_theta, 2/mu, length.out = (1440 * 14)/160+1)
# theta_vec = theta_vec[-1]
# theta_vec_2 = 1/seq(min(1/theta_vec), max(1/theta_vec), length.out = 100)
# theta_vec = c(theta_vec, theta_vec_2) %>% unique()
...
BFD_range = all_vary_parameters %>% 
  mutate(BFD = 1/lQ + 1/lL + 1/lP + 1/lG) %>% 
  select(BFD) %>% range()

vary_exp_parameters = tibble(
  cbind(extra_parameters,
        dplyr::select(parameters, -theta),
        tibble(theta = 1/seq(1/BFD_range[2], 1/BFD_range[1], length.out = 1001))),
  type = "Exponential")

# vary_exp_parameters = all_vary_parameters %>% 
#   mutate(theta = 1/lQ + 1/lL + 1/lP + 1/lG) %>% 
#   mutate(type = "Exponential") %>% 
#   dplyr::select(-(pL:lQ), -vary_parm) %>%
#   unique()
