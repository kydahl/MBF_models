...
Figure5_separated_nonmech <- Figure5_df |> 
  filter(`Model type` != "Mechanistic") |> 
  ...
  # Add ticks for R0 = 1 crossing below the x-axis
  geom_rug(
    data = filter(Figure5_ticks, !(Type %in% c("Mechanistic~(lambda[Q])", "Mechanistic~(p[G])", "Mechanistic~(p[P])"))),
    aes(x = first_R0_greater_1),
    sides = "b", size = 0.75, outside = TRUE,
    length = unit(0.3, "in"),
    show.legend = F
...
Figure5_separated_mech <- Figure5_df |> 
  filter(`Model type` == "Mechanistic") |> 
  filter(sbr < 1.6) |> # only keep values up to 2 bites per day
  ggplot(aes(color = Type, lty = Type)) +
  # Grey line for R0 = 1
...
Figure5_separated_mech

Figure5_separated = egg::ggarrange(Figure5_separated_nonmech, Figure5_separated_mech, ncol = 1)

ggsave("figures/Figure5_separated.pdf", Figure5_separated, width = 7.5, height = 3.25 * 9/6.5, units = "in")

# Table 5: Characteristics of R0 curves ----
...
# Figure S1: eFAST indices of N0 and R0 wrt parameters ----
# Load in data
...
ggsave("figures/FigureS1.pdf", FigureS1, width = 6.5, height = 2.25 * 9/6.5, units = "in")