...
# Labels
plot_df$Prefix = factor(plot_df$Prefix, levels = c("Exponential", "Detection", "Seek-new-host", "Landing", "Probing", "Ingesting"))
...
alt_GCD_contact_OUT_host_plot <- GCD_contact_OUT_df %>% 
  ...
  geom_line(
    data = GCD_contact_OUT_df %>%
      group_by(varied_parameter) %>%
      filter(between(GCD_day, nudge0, nudge1)),
    lwd = 1,
    arrow = arrow(ends = "first", type = "closed")
  ) +
  geom_line(
    data = GCD_contact_OUT_df %>% 
      group_by(varied_parameter) %>% 
      filter(between(GCD_day, nudge1, nudge2)),
    lwd = 1,
    arrow = arrow(ends = "first", type = "closed")
  ) +
  geom_line(
    data = GCD_contact_OUT_df %>% 
      group_by(varied_parameter) %>% 
      filter(between(GCD_day, nudge2, nudge3)),
    lwd = 1,
    arrow = arrow(ends = "first", type = "closed")
  ) +
  geom_line(
    data = GCD_contact_OUT_df %>% 
      group_by(varied_parameter) %>% 
      filter(between(GCD_day, nudge3, nudge4)),
    lwd = 1,
    arrow = arrow(ends = "first", type = "closed")
  ) +
  facet_wrap(vars(Type), ncol = 1, scales = "free") +
...
alt_R0_invGCD_OUT_plot <- R0_invGCD_OUT_df %>% 
  ...
  geom_line(
    data = R0_invGCD_OUT_df %>%
      group_by(varied_parameter) %>%
      filter(between(invGCD, nudge3 - 0.01, nudge3)),
    lwd = 1,
    arrow = arrow(ends = "first", type = "closed")
  ) +
  geom_line(
